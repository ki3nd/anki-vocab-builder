...
permissions:
  allow:
    - Write(tables/**)
    - Exec(uv run .devin/skills/build-phrasal-verbs-table/fetch_word.py *)
    - Exec(mkdir *)
triggers:
  - user
---
...
@skills/build-phrasal-verbs-table/fetch_word.py
...
- **ipa** — `pronunciations[0].text` (from the entry; leave empty if none)
