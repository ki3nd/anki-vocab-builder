...
permissions:
  allow:
    - Write(tables/**)
    - Exec(uv run .devin/skills/build-vocab-table/fetch_word.py *)
    - Exec(mkdir *)
triggers:
  - user
---
...
@skills/build-vocab-table/fetch_word.py
...
- **ipa** — `pronunciations[0].text` (from the entry, already filtered); if empty, write the IPA transcription yourself (General American preferred)
